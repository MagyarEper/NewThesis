...
def main():
    ...
    if not xlsx_path.exists():
        print(f"ERROR: Az xlsx fájl nem található: {xlsx_path}", file=sys.stderr)
        sys.exit(1)
    
    if not wav_base_dir.exists():
        print(f"WARNING: A WAV könyvtár nem létezik: {wav_base_dir}")
    ...
    if missing_cols:
        print(f"ERROR: Hiányzó oszlopok: {missing_cols}", file=sys.stderr)
        sys.exit(1)
